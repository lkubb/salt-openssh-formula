...
We will see a quick setup to configure the ``openssh`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``openssh`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``openssh`` formulas under ``/srv/salt/openssh/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/openssh/parameters/dns:domain/
    ln -s dns:domain /srv/salt/openssh/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/openssh/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/openssh/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``openssh`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``openssh`` formulas under ``/srv/salt/openssh/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/openssh/parameters/roles
...
We create a configuration for the role ``openssh/server`` in ``/srv/salt/openssh/parameters/roles/openssh/server.yaml``:
...
We create another configuration for the role ``openssh/client`` in ``/srv/salt/openssh/parameters/roles/openssh/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles openssh/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles openssh/client
...
.. code-block:: console
    ...
    mv /srv/salt/openssh/parameters/dns:domain/  '/srv/salt/openssh/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-openssh-as-json:
      test.nop:
        - name: {{ openssh | json }}
...
Here is an example based on `template-formula/openssh/config/file.sls`_:
    ...
    openssh-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='openssh-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ openssh.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``openssh`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ openssh['winner'] }}
...
.. _template-formula/openssh/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/openssh/config/file.sls
